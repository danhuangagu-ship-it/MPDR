...
parser.add_argument("--q_start", required=True, help="Path to q_test.csv (e.g., q_disease_perm.csv)")
...
# Align q_test to (n_test x F)
if Q_test.shape[0] == N_f:
    Q2 = torch.from_numpy(Q_test.T.astype(np.float32))
elif Q_test.shape[1] == N_f:
    Q2 = torch.from_numpy(Q_test.astype(np.float32))
else:
    raise ValueError(f"q_test shape incompatible with q_train F={N_f}: q_test={Q_test.shape}")
